...
public class Snippet034ComboViewerAndEnum {
	...
	static class View {
		...
		public Shell createShell() {
			...
			IObservableValue widgetObservable = SWTObservables.observeText(name, SWT.Modify);
			bindingContext.bindValue(widgetObservable, PojoObservables.observeValue(viewModel, "name"));

			// The second key to binding a combo to an Enum is to use a
			// selection observable from the ComboViewer:
			widgetObservable = ViewersObservables.observeSingleSelection(gender);
			bindingContext.bindValue(widgetObservable, PojoObservables.observeValue(viewModel, "gender"));
