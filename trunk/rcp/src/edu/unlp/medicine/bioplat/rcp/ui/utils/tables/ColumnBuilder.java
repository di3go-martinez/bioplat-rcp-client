...
public class ColumnBuilder {
	...
	void build(TableViewer viewer, int index) {
        TableViewerColumn tvc = createTableViewerColumn(viewer, title, width,
				index, alignStyle);
	...
	private EditingSupport newInstance(Class<? extends EditingSupport> clazz,
			TableViewer viewer) {
		try {
            // TODO revisar... cuando se acomode la clase a recibir deber�a
            // extender a AbstractEditingSupport
			if (accesor == null) {
				return clazz.getConstructor(TableViewer.class).newInstance(
						viewer);
			} else {
				return clazz.getConstructor(TableViewer.class, Accesor.class)
						.newInstance(viewer, accesor);
			}
	...
	private TableViewerColumn createTableViewerColumn(TableViewer viewer,
			String title, int bound, final int colNumber, int alignStyle) {
		final TableViewerColumn viewerColumn = new TableViewerColumn(viewer,
				alignStyle);
		final TableColumn column = viewerColumn.getColumn();
		column.setText(title);
		column.setWidth(bound);
		column.setResizable(true);// TODO
    ...
    public ColumnBuilder cellEditorSupport(
    ...
    public ColumnBuilder mutator(Accesor mutator) {
