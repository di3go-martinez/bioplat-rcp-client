 ...
 */
public class BlindSearchResultViewPart extends GenericOptimizationResultViewPart {
	...
	// MenuItemContribution.create(menu).image(openImage).text("Open Selection").addSelectionListener(new
	// SelectionAdapter() {
	// @Override
	...
	// TableBuilder.create(container).input(betters).addColumn(ColumnBuilder.create().property("name").title("Gene Signature name").width(200));
	...
	// ColumnBuilder.create().numeric().title(title).width(150);
	//
	...
	// return
	// String.valueOf(StatisticConfig.getBiomarkerFitness((Biomarker)biomarker,
	// forValidation));
	// }
	//
	// });
	...
	// tableBuilder.addColumn(ColumnBuilder.create().numeric().title(title).width(150).labelprovider(new
	// ColumnLabelProvider() {
	// @Override
	// public String getText(Object biomarker) {
	// return
